...
module.exports = {
  name: "youtube",
  description: "Inicia uma sessão do YouTube Together meu cupinxa",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    if (!message.member.voice.channel)
      return client.sendTime(
        message.channel,
        "❌ | **Tu tem que estar em um canal pra usar esse comando meu cupinxa**"
      );
    if (
      ...
      return client.sendTime(
        message.channel,
        "❌ | **Ae meu cupinxa, eu não tenho permissão taligado**"
      );
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!member.voice.channel)
        return client.sendTime(
          interaction,
          "❌ | Tem que estar no mesmo canal que eu pra usar esse comando meu cupinxa."
        );
      if (
