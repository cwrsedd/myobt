...
module.exports = {
  name: "search",
  description: "Mostra os resultados de um som baseado na busca",
  usage: "[song]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Feio, se usa assim - **\`${GuildDB.prefix}search [query]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      return client.sendTime(
        message.channel,
        "Feio, não achei nada pra " + SearchString
      );
    else {
      ...
      let msg = await message.channel.send(
        "**Feio me diz, escolhe o numero da musica que você quer dar play. Você tem `30 segundos`.**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**A feio vacilasse, o tempo passou e tu não escolheu**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Feio da pra tu mandar o ID certo?"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "Feio eu não achei nenhum som com esse ID"
        );
      player.queue.add(Song);
      if (!player.playing && !player.paused && !player.queue.size)
        player.play();
      let SongAddedEmbed = new MessageEmbed();
      SongAddedEmbed.setAuthor(`Adicionei na fila`, client.botconfig.IconURL);
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          ...
          case "TRACK_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Adicionei na fila:** \`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri}}\`.`
            );
          ...
          case "PLAYLIST_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Feio adicionei a playlist na fila**: \n**${Searched.playlist.name}** \nEnfileirados: **${Searched.playlistInfo.length} musicas**`
            );
        }
      } else {
        try {
          ...
          return client.sendTime(
            interaction,
            `❌ | **Feio deu erro na busca aqui:** ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
          ...
          case "TRACK_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Feio adicionei na fila:** \`[${res.tracks[0].title}](${res.tracks[0].uri})\`.`
            );
          case "PLAYLIST_LOADED":
            ...
            return client.sendTime(
              interaction,
              `**Feio adicionei a playlist na fila**: \n**${res.playlist.name}** \nEnfileirado: **${res.playlistInfo.length} musicas**`
            );
          case "SEARCH_RESULT":
            let max = 10,
              collected,
            ...
            const resultss = new MessageEmbed()
              .setDescription(
                `${results}\n\n\t**Feio escreve o numero do som que tu quer tocar**\n`
              )
              .setColor(client.botconfig.EmbedColor)
              .setAuthor(
            ...
            try {
              ...
              return awaitchannel.send(
                "❌ | **Feio tu não forneceu uma seleção**"
              );
            }
            ...
            if (first.toLowerCase() === "cancel") {
              if (!player.queue.current) player.destroy();
              return awaitchannel.send("Cancelei a busca feio.");
            }

            const index = Number(first) - 1;
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Feio o número que você forneceu foi maior ou menor que o total da pesquisa. É assim que se usa - \`(1-${max})\``
              );
